...
```{r setup images folder, include=FALSE, message=FALSE, eval=TRUE}
# http://lcolladotor.github.io/2018/03/07/blogdown-insert-image-addin/
images_folder <- paste0("2020-11-28-", "politely-scraping-health-insurance-data")
dir.create(here::here("static", "post", images_folder, "index_files"), showWarnings = FALSE)
path_to_project <- file.path("C:", "Users", "jfang", "Dropbox", "Projects", "meps-ic")

current.folder <- file.path(path_to_project, "polite", "media")
new.folder <- here::here("static", "post", images_folder, "index_files")

# find the files that you want
list.of.files <- list.files(current.folder)

# copy the files to the new folder
file.copy(file.path(current.folder, list.of.files), file.path(new.folder, list.of.files), overwrite = TRUE, copy.mode = TRUE, recursive = TRUE)
```

```{r path to data, include=FALSE, message=FALSE}
path_to_data <- file.path("C:", "Users", "jfang", "Dropbox", "Projects", "meps-ic", "polite")
```
...
![](`r paste0('/post/', images_folder, '/index_files/meps-banner.PNG')`)
...
```{r toc prep, warning=FALSE, message=FALSE}
toc <- 
  toc_df %>% 
  select(-update) %>% 
  separate(title,
...
As part of the project, I'm interested in the trends in the total premium costs of family health insurance through an employer (the sum of employer and employee contributions). To pull this data, I need to scrape the data from Table IID2, which has this data by state. First, I pull a vector of URLs from the table of contents, and then I use the `politely` function to apply polite principles in `purrr::map` style with the `httr::GET` function.

```{r scrap prem, eval=FALSE}
...
![](`r paste0('/post/', images_folder, '/index_files/family-prem.gif')`)
